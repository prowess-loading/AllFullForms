...
class SmoothScroll:
    ...
    def _scroll(self, scroll_amount, total_scroll_height, element=None):
        if element:
            ...
            for position in range(int(current_position), int(next_position), int(scroll_step)):
                self.driver.execute_script(
                    f"arguments[0].scrollTop = {min(position, total_scroll_height)};", element)
                time.sleep(0.03 + random.uniform(-0.03, 0.03))

            return self.driver.execute_script("return arguments[0].scrollTop;", element)
        else:
            current_position = self.driver.execute_script(
    ...
    def scroll_bottom_up_button_click(self, by=By.CSS_SELECTOR):
        ...
        scroll_element = self.driver.find_element(by, ".content")
        ...
        while True:
            ...
            current_position = self.driver.execute_script(
                "return arguments[0].scrollTop;", scroll_element)
            total_scroll_height = self.driver.execute_script(
            ...
            scroll_amount = - \
                random.randint(
                    80, 150) if scrolling_up else random.randint(500, 900)
            new_position = self._scroll(
                scroll_amount, total_scroll_height, element=scroll_element)
